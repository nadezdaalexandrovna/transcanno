class TranscribeController  < ApplicationController
  ...
  def display_page
    ...
    sqlSeq="SELECT categoryattributes.category_id, categoryattributes.id, attributevalues.value, attributecats.name, valuestoattributesrelations.consequent_attr_id, valuestoattributesrelations.attributevalue_id, valuestoattributesrelations.id,  attributecats_initial.name, attributecats_initial.id FROM valuestoattributesrelations INNER JOIN attributevalues ON attributevalues.id = valuestoattributesrelations.attributevalue_id INNER JOIN attributecats ON attributecats.id=valuestoattributesrelations.consequent_attr_id INNER JOIN attributes_to_values ON attributes_to_values.attributevalue_id=attributevalues.id INNER JOIN categoryattributes ON attributes_to_values.categoryattribute_id=categoryattributes.id INNER JOIN attributecats attributecats_initial ON attributecats_initial.id=categoryattributes.attributecat_id INNER JOIN categories ON categories.id=categoryattributes.category_id INNER JOIN works on categories.collection_id=works.collection_id INNER JOIN pages ON pages.work_id=works.id WHERE pages.id="+params[:page_id];
